...
canonical `{parent_dir}_{stem}` spec (one parent level, no extension) so AST and
semantic extraction produce the SAME node for a file instead of two disconnected
ghosts.

canonical node-ID contract:
...
def test_top_level_file_SYMBOL_ids_use_bare_stem(tmp_path):
    ...
    semantic extraction emits the bare-stem canonical form, so an absolute-parent
