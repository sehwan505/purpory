"""Tests for `purpory reflect` and the work-memory reflection layer.

`purpory reflect` reads the outcome-tagged Q&A docs that `purpory save-result`
files into purpory-out/memory/ and writes a deterministic lessons artifact
(purpory-out/reflections/LESSONS.md) an agent can load next session: preferred
...
from purpory.ingest import save_query_result
from purpory.reflect import (
    aggregate_lessons,
    lessons_fresh,
    load_memory_docs,
    parse_memory_doc,
...
def _write_raw_doc(mem: Path, filename: str, date: str, *, outcome="dead_end",
    ...
    lines = ["---", 'type: "query"', f'date: "{date}"', f'question: "{question}"',
             'contributor: "purpory"', f'outcome: "{outcome}"']
    if nodes:
...
def test_cli_reflect_cold_start_writes_empty_lessons(tmp_path):
    """First run with no purpory-out/memory/ still succeeds and writes a valid doc."""
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels in tmp_path/purpory-out/.
...
@pytest.mark.parametrize("sidecar_name", [".purpory_analysis.json", ".purpory_labels.json"])
def test_lessons_fresh_false_when_graph_sidecar_newer(tmp_path, sidecar_name):
    ...
    out = tmp_path / "LESSONS.md"; out.write_text("y", encoding="utf-8")
    graph = tmp_path / "graph.json"; graph.write_text("{}", encoding="utf-8")
    analysis = tmp_path / ".purpory_analysis.json"; analysis.write_text("{}", encoding="utf-8")
    labels = tmp_path / ".purpory_labels.json"; labels.write_text("{}", encoding="utf-8")
...
# --- work-memory overlay sidecar (.purpory_learning.json) --------------------
#
...
from purpory.reflect import (  # noqa: E402
...
def test_sidecar_write_classifies_and_keys_by_canonical_id(tmp_path):
    """reflect with a graph writes .purpory_learning.json next to graph.json with
...
def test_relative_source_file_not_spuriously_stale_in_purpory_out_layout(tmp_path):
    """Regression: with a RELATIVE source_file and graph.json under purpory-out/,
    ...
    (purpory-out/) — otherwise every node looked unfindable and was marked stale.
    The edit case must still flip stale=True."""
    out = tmp_path / "purpory-out"          # graph.json lives here
...
def test_relative_source_file_resolved_via_purpory_root_marker(tmp_path):
    """When a committed .purpory_root marker records the project root (e.g. a
    PURPORY_OUT override pointing the output dir elsewhere), the fingerprint
    ...
    (out / ".purpory_root").write_text(str(proj), encoding="utf-8")  # the marker
    mem = out / "memory"
    _write_raw_doc(mem, "a.md", "2026-05-01", outcome="useful", nodes=["login()"])
    _write_raw_doc(mem, "b.md", "2026-05-10", outcome="useful", nodes=["login()"])
    reflect(mem, out / "reflections" / "LESSONS.md",
...
def test_flat_layout_does_not_match_same_named_file_one_dir_up(tmp_path):
    ...
    # Flat layout: graph.json sits directly in proj/ (not a purpory-out subdir).
