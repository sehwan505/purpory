"""Tests for `purpory extract` CLI dispatch path in purpory.__main__."""
...
def test_extract_exits_nonzero_when_all_semantic_chunks_fail(
    ...
    The bug this guards: `pip install purpory` doesn't pull in `anthropic`,
    so `purpory extract --backend claude` would print per-chunk errors and
    ...
    monkeypatch.setattr(
        "purpory.llm.extract_corpus_parallel", _all_chunks_failed
    )
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
...
def test_extract_succeeds_when_at_least_one_chunk_completes(
    ...
    monkeypatch.setattr(
        "purpory.llm.extract_corpus_parallel", _one_chunk_succeeded
    )
    cache_call = {}
    ...
    monkeypatch.setattr(
        "purpory.cache.save_semantic_cache", _capture_semantic_cache
    )
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
...
def test_incremental_partial_run_preserves_untouched_semantic_hash(
    ...
    monkeypatch.setattr(
        "purpory.llm.extract_corpus_parallel", _stamp_everything_sent
    )
...
def test_truncated_doc_semantic_hash_is_cleared_for_requeue(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("purpory.llm.extract_corpus_parallel", _extract)
...
def test_manifest_stamps_freshly_extracted_semantic_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("purpory.llm.extract_corpus_parallel", _fresh_relative)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(corpus), "--backend", "claude",
         "--no-cluster", "--out", str(out_dir)],
...
def test_manifest_stamps_hyperedge_only_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("purpory.llm.extract_corpus_parallel", _hyperedge_only)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(corpus), "--backend", "claude",
         "--no-cluster", "--out", str(out_dir)],
    ...
    manifest = json.loads((out_dir / "purpory-out" / "manifest.json").read_text())
...
def test_extract_mode_deep_dispatches_over_warm_cache(monkeypatch, tmp_path):
    ...
    corpus = _make_corpus(tmp_path)
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-test-fake-key")
    monkeypatch.delenv("PURPORY_FORCE", raising=False)
    calls: list[dict] = []
    monkeypatch.setattr("purpory.llm.extract_corpus_parallel",
                        _recording_extractor(calls))

    # No --out: the default layout (purpory-out/ beside the sources) keeps the
    ...
    base = ["purpory", "extract", str(corpus), "--backend", "claude",
            "--no-cluster"]
...
def test_extract_purpory_force_env_redispatches(monkeypatch, tmp_path):
    """PURPORY_FORCE=1 behaves like --force (env parity with `update`)."""
    corpus = _make_corpus(tmp_path)
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-test-fake-key")
    monkeypatch.delenv("PURPORY_FORCE", raising=False)
    calls: list[dict] = []
    ...
    monkeypatch.setenv("PURPORY_FORCE", "1")
    _run_extract(monkeypatch, base)
...
def test_extract_codeonly_succeeds_without_api_key(monkeypatch, tmp_path):
    ...
    Regression: purpory extract validated a backend upfront and exited 1 with
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(corpus), "--out", str(out_dir)],
    )
...
def test_missing_manifest_code_only_preserves_semantic_layer(monkeypatch, tmp_path):
    """#1925: `purpory extract --code-only` with a MISSING manifest.json must
    ...
    (corpus / "README.md").write_text("# Notes\nCurated docs.\n")
    out_dir = tmp_path / "out"
    purpory_out = out_dir / "purpory-out"
    _clear_backend_keys(monkeypatch)
...
def test_extract_out_keeps_project_root_clean(monkeypatch, tmp_path):
    """`extract --out DIR` routes every artifact to DIR/purpory-out/ and the
    scanned project must not grow a purpory-out/ (or anything else) beside
    its sources.
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", ".", "--out", str(external)],
    )
...
def test_extract_without_key_still_errors_when_docs_present(
    ...
    monkeypatch.setattr("purpory.llm.detect_backend", lambda: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(corpus), "--out", str(out_dir)],
    )
...
def test_extract_timing_flag_emits_stage_timings(monkeypatch, tmp_path, capsys):
    """--timing prints per-stage `[purpory timing]` lines to stderr (#1490); omitting
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o1"), "--timing"],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
    err = capsys.readouterr().err
    assert "[purpory timing] detect:" in err
    assert "[purpory timing] total:" in err

    # without --timing => no timing lines
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o2")],
    )
    with pytest.raises(SystemExit) as exc2:
        mainmod.main()
    assert exc2.value.code == 0
    assert "purpory timing" not in capsys.readouterr().err
...
def test_incremental_extract_prunes_newly_excluded_file_not_in_manifest(
    ...
    .purporyignore, re-run extract: x.py's nodes must be gone even though it
...
def test_no_cluster_incremental_prunes_newly_excluded_file(
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["purpory", "extract", str(project), "--no-cluster", "--out", str(out_dir)],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
