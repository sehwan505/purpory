...
Both callers now delegate to :mod:`purpory.ids`, so they share one
...
from purpory.build import _normalize_id
from purpory.extract import _make_id
from purpory.ids import make_id, normalize_id
...
def test_both_callers_share_one_implementation():
    ...
    underlying function object as purpory.ids.normalize_id."""
    # build._normalize_id is imported directly from purpory.ids.
    ...
    from purpory.mcp_ingest import _make_id as _mcp_make_id
    from purpory.symbol_resolution import _bash_make_id
