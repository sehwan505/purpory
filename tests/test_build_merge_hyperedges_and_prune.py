...
build_merge backs `purpory --update`. Two regressions covered here:
...
from purpory.build import build_merge, _infer_merge_root
...
def test_prune_without_root_removes_ghost_nodes_via_grandparent_fallback(tmp_path):
    root = tmp_path / "corpus"
    (root / "purpory-out").mkdir(parents=True)
    graph_path = root / "purpory-out" / "graph.json"
    nodes = [
...
def test_prune_without_root_uses_purpory_root_marker(tmp_path):
    # graph.json not under a <root>/purpory-out layout, so grandparent wouldn't
    # help — the committed .purpory_root marker must be honored instead.
    out = tmp_path / "out"
    out.mkdir()
    graph_path = out / "graph.json"
    real_root = tmp_path / "elsewhere" / "repo"
    real_root.mkdir(parents=True)
    (out / ".purpory_root").write_text(str(real_root), encoding="utf-8")
...
def test_prune_matches_across_symlinked_root(tmp_path):
    ...
    real = tmp_path / "real"
    (real / "purpory-out").mkdir(parents=True)
    link = tmp_path / "link"
    os.symlink(real, link)
    graph_path = real / "purpory-out" / "graph.json"
