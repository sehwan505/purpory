...
def _store_graph(root: Path, graph: dict) -> None:
...
def test_purpory_root_preserves_relative_when_invoked_with_relative_path(tmp_path, monkeypatch):
    """#777: ``.purpory_root`` stores the user-supplied path (``.``), not the
    resolved absolute, so a committed ``purpory-out/.purpory_root`` is
...
def test_rebuild_code_writes_community_name(tmp_path):
    """#1808: `purpory update` / _rebuild_code must forward community_labels to
...
def test_update_rebuilds_with_nested_star_gitignore(tmp_path):
    """#1880: `purpory update` must not emit 0 nodes (and then refuse to
...
def test_update_discovers_newly_added_files_and_dirs(tmp_path):
    """#1837: after an initial build, a plain `purpory update` (full re-scan, no
    ...
    sources = {n.get("source_file", "") for n in _graph(corpus)["nodes"]}
...
def test_rebuild_honors_persisted_excludes(tmp_path):
    ...
    import json
    from purpory.watch import _rebuild_code, _write_build_config
...
def test_purpory_root_preserves_absolute_when_user_supplied(tmp_path):
    """When the caller supplies an absolute path, ``.purpory_root`` stores
...
def test_rebuild_code_deleted_cwd_without_repo_root_returns_false(tmp_path, monkeypatch, capsys):
    ...
    Without PURPORY_REPO_ROOT, the rebuild should fail cleanly before creating
    relative purpory-out queue/lock files.
    """
    from purpory.watch import _rebuild_code
    ...
    gone = tmp_path / "gone"
    gone.mkdir()
    monkeypatch.delenv("PURPORY_REPO_ROOT", raising=False)
...
def test_rebuild_code_deleted_cwd_uses_purpory_repo_root(tmp_path, monkeypatch):
    """PURPORY_REPO_ROOT lets detached hook rebuilds recover from a deleted CWD."""
    from purpory.watch import _rebuild_code
    ...
    gone = tmp_path / "gone"
    gone.mkdir()
    monkeypatch.setenv("PURPORY_REPO_ROOT", str(corpus))
    ...
    try:
        ...
        assert _graph(corpus)["nodes"]
    finally:
        os.chdir(old_cwd)


def test_rebuild_code_evicts_nodes_from_deleted_files(tmp_path):
    """#1007: purpory update (_rebuild_code with no changed_paths) must remove
...
def _add_unrelated_semantic_pair(root):
    data = _graph(root)
    data["nodes"].extend([
...
def test_rebuild_code_prunes_final_deleted_file(tmp_path, changed_paths):
    ...
    assert _rebuild_code(corpus, no_cluster=True, acquire_lock=False) is True
    _add_unrelated_semantic_pair(corpus)
...
def test_rebuild_code_evicts_removed_symbol_from_surviving_file(tmp_path):
    ...
    # shrink, so the shrink-guard must let `purpory update` refresh the graph
    # without --force (the lost node belongs to a rebuilt source).
    assert _rebuild_code(corpus, acquire_lock=False) is True
    after_data = _graph(corpus)
...
def test_rebuild_code_is_idempotent_when_cluster_ids_flap(tmp_path, monkeypatch):
    from purpory import cluster as cluster_mod
    from purpory.watch import _rebuild_code
    ...
    first_graph = _graph(tmp_path)

    assert _rebuild_code(tmp_path)
    second_graph = _graph(tmp_path)
...
# --- .purporyignore honored in watch handler (gh-928) ---
...
def test_watch_handler_honors_purporyignore(tmp_path, monkeypatch):
    """gh-928: the watch Handler must short-circuit paths matching
    .purporyignore so busy volumes (node_modules churn, build artefacts,
    ...
    (watch_root / ".purporyignore").write_text("node_modules/\nbuild/\n", encoding="utf-8")
...
@pytest.mark.skipif(not _watchdog_available(), reason="watchdog not installed")
def test_watch_loads_purporyignore_once(tmp_path, monkeypatch):
    """gh-928: .purporyignore must be parsed exactly once at watch() startup,
    ...
    (tmp_path / ".purporyignore").write_text("ignored/\n", encoding="utf-8")
    (tmp_path / "ignored").mkdir()

    calls = {"n": 0}
    real_loader = detect_mod._load_purporyignore
    ...
    time.sleep(0.7)
    assert calls["n"] == 1, f"_load_purporyignore called {calls['n']} times; expected 1"
...
def test_rebuild_code_incremental_rename_preserves_symlink_source_path(tmp_path):
    ...
    real = corpus / "real"
    real.mkdir()
    (corpus / ".purporyignore").write_text("real/\n", encoding="utf-8")
    old = real / "old.py"
...
def test_merge_changed_paths_dedupes_in_order():
    ...
    from purpory.watch import _merge_changed_paths
...
def _seed_semantic_doc_graph(corpus):
    ...
    Mimics a graph produced by the CLI ``purpory . --update`` path: code AST
...
def test_rebuild_code_semantic_doc_not_double_represented_on_full_rebuild(tmp_path):
    ...
    corpus = tmp_path / "corpus"
    _seed_semantic_doc_graph(corpus)
    before = _graph(corpus)
...
def test_rebuild_code_concept_only_semantic_doc_not_double_represented_on_full_rebuild(
    ...
    corpus = tmp_path / "corpus"
    _seed_semantic_doc_graph_concept_only(corpus)
    before = _graph(corpus)
...
def test_rebuild_code_quick_scans_doc_without_semantic_nodes(tmp_path):
    ...
    assert _rebuild_code(corpus, no_cluster=True, acquire_lock=False) is True
    ids = {n["id"] for n in _graph(corpus)["nodes"]}
