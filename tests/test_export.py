...
def test_to_html_neighbor_links_have_no_inline_onclick_xss():
    ...
    (from a document or a scraped `purpory add` URL) inject a live event handler
...
# ── Existing-vault safety: purpory must not clobber user notes / .obsidian (#1506) ──
...
def test_to_obsidian_preserves_existing_user_notes_and_obsidian_config():
    ...
    collides with a purpory node name, nor their .obsidian/ graph settings."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # non-colliding purpory note still written
...
def test_to_obsidian_rerun_updates_own_notes_but_not_user_files():
    """A re-run overwrites purpory's own prior notes (via the manifest) but leaves a
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (out / "Database.md").exists()  # purpory re-wrote its own
...
def test_to_obsidian_rerun_prunes_removed_nodes():
    """#1896: re-exporting into the same vault must delete purpory's own notes for
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # surviving purpory notes and the user's own note remain
        assert (out / "Database.md").exists() and (out / "Server.md").exists()
...
def test_backup_semantic_marker(tmp_path):
    """graph.json + .purpory_semantic_marker → backup taken."""
    from purpory.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / "GRAPH_REPORT.md").write_text("# Report")
    (tmp_path / ".purpory_semantic_marker").write_text('{"output_tokens": 1234}')
    result = backup_if_protected(tmp_path)
    assert result is not None
    assert result.is_dir()
    assert (result / "graph.json").exists()
    assert (result / "GRAPH_REPORT.md").exists()
    assert (result / ".purpory_semantic_marker").exists()


def test_backup_curated_labels(tmp_path):
    """graph.json + non-default label in .purpory_labels.json → backup taken."""
    import json
    from purpory.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".purpory_labels.json").write_text(json.dumps({"0": "Auth Pipeline", "1": "Community 1"}))
    result = backup_if_protected(tmp_path)
    assert result is not None
...
def test_backup_default_labels_only(tmp_path):
    ...
    from purpory.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".purpory_labels.json").write_text(json.dumps({"0": "Community 0", "1": "Community 1"}))
    assert backup_if_protected(tmp_path) is None
...
def test_backup_env_disable(tmp_path, monkeypatch):
    """PURPORY_NO_BACKUP=1 disables backup entirely."""
    from purpory.export import backup_if_protected
    monkeypatch.setenv("PURPORY_NO_BACKUP", "1")
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".purpory_semantic_marker").write_text("{}")
    assert backup_if_protected(tmp_path) is None
...
def test_existing_graph_node_count(tmp_path):
    from purpory.export import existing_graph_node_count, MALFORMED_GRAPH
    p = tmp_path / "graph.json"
