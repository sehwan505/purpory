...
def test_affected_cli_reverse_traverses_impact_edges(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["purpory", "affected", "Foo", "--graph", str(graph_path)],
    )
...
def test_affected_cli_relation_filter_limits_reverse_traversal(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["purpory", "affected", "Foo", "--relation", "calls", "--graph", str(graph_path)],
    )
...
def test_affected_cli_forces_directed_on_undirected_graph(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["purpory", "affected", "B", "--relation", "calls", "--graph", str(graph_path)],
    )
...
def test_affected_cli_loads_edges_keyed_graph(monkeypatch, tmp_path, capsys):
    """purpory's `extract` writes graph.json with an "edges" key (not networkx's
    ...
    # Emulate purpory extract output: top-level "edges" key instead of "links".
    data = json_graph.node_link_data(graph, edges="links")
...
def test_resolve_seed_bare_name_matches_callable_label():
    from purpory.affected import resolve_seed
...
def test_affected_cli_source_file_path_uses_file_level_node(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["purpory", "affected", source_file, "--graph", str(graph_path)],
    )
