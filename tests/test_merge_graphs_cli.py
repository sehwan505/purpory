"""`purpory merge-graphs` tolerates inputs that disagree on graph type (#1606).
...
def _run(args, cwd):
    return subprocess.run([PYTHON, "-m", "purpory"] + args, cwd=cwd,
...
def test_merge_graphs_mixed_directed_and_multigraph(tmp_path):
    a = tmp_path / "r1" / "purpory-out" / "graph.json"
    b = tmp_path / "r2" / "purpory-out" / "graph.json"
    c = tmp_path / "r3" / "purpory-out" / "graph.json"
...
def test_merge_graphs_same_named_repo_dirs_do_not_collapse(tmp_path):
    # #1729: two graphs under a same-named repo dir (src/purpory-out and
    # frontend/src/purpory-out both → tag "src") share the `src::` prefix, so a
    ...
    a = tmp_path / "src" / "purpory-out" / "graph.json"
    b = tmp_path / "frontend" / "src" / "purpory-out" / "graph.json"
...
def test_distinct_repo_tags_unit(tmp_path):
    from purpory.build import distinct_repo_tags
    # distinct repo dirs pass through unchanged
    assert distinct_repo_tags([
        Path("backend/purpory-out/graph.json"),
        Path("web/purpory-out/graph.json"),
    ]) == ["backend", "web"]
    # same-named repo dirs are widened to stay distinct
    tags = distinct_repo_tags([
        Path("proj/src/purpory-out/graph.json"),
        Path("proj/frontend/src/purpory-out/graph.json"),
    ])
    ...
    tags3 = distinct_repo_tags([
        Path("a/src/purpory-out/graph.json"),
        Path("b/src/purpory-out/graph.json"),
        Path("c/src/purpory-out/graph.json"),
    ])
