...
from purpory import serve as serve_mod  # noqa: E402
...
def test_app_builds_and_initialize_succeeds(tmp_path):
    ...
    with _client(app) as client:
        ...
        assert payload["result"]["serverInfo"]["name"] == "purpory"
...
def test_api_key_bearer_ok(tmp_path):
    ...
    with _client(app) as client:
        ...
        assert resp.status_code == 200
        assert resp.json()["result"]["serverInfo"]["name"] == "purpory"
...
def _project_with_graph(tmp_path, node_count: int) -> str:
    """Create ``<proj>/purpory-out/graph.json`` and return the project dir."""
    proj = tmp_path / "proj"
    (proj / "purpory-out").mkdir(parents=True)
    graph = {
        "directed": True,
        "nodes": [{"id": f"n{i}", "label": f"N{i}", "community": 0} for i in range(node_count)],
        "edges": [],
    }
    (proj / "purpory-out" / "graph.json").write_text(json.dumps(graph), encoding="utf-8")
    return str(proj)
...
def test_cli_api_key_from_env(monkeypatch):
    captured = {}
    monkeypatch.setenv("PURPORY_API_KEY", "from-env")
