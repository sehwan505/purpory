...
from purpory.callflow_html import derive_sections_from_communities, write_callflow_html


def _make_purpory_out(tmp_path: Path) -> Path:
...
def test_write_callflow_html_creates_file_and_uses_report(tmp_path):
    out = _make_purpory_out(tmp_path)

    html_path = write_callflow_html(
        tmp_path,
        output="purpory-out/callflow.html",
        max_sections=4,
    )
...
def test_export_callflow_html_cli_creates_file(tmp_path):
    ...
    html_path = tmp_path / "purpory-out" / "from-cli.html"
...
def test_export_callflow_html_cli_accepts_positional_graph_path(tmp_path):
    _make_purpory_out(tmp_path)
    external_out = tmp_path / "GitNexus" / "purpory-out"
    ...
    (external_out / ".purpory_labels.json").write_text(json.dumps({"0": "External Runtime", "1": "External Export"}), encoding="utf-8")
...
def test_derive_sections_groups_by_architecture_keywords():
    nodes = [
        {"id": "extract_py", "label": "extract_python", "source_file": "purpory/extract.py", "community": 0},
        {"id": "extract_js", "label": "extract_js", "source_file": "purpory/extract.py", "community": 0},
        {"id": "to_html", "label": "to_html", "source_file": "purpory/export.py", "community": 1},
...
def test_load_graph_rejects_oversized_file(monkeypatch, tmp_path):
    ...
    import pytest
    from purpory.callflow_html import load_graph
