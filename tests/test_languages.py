...
def test_metal_is_code_extension():
    from purpory.detect import CODE_EXTENSIONS
...
def test_kotlin_emits_in_file_calls():
    ...
    `identifier` rename — see purpory-kmp's PythonParityTest."""
    r = extract_kotlin(FIXTURES / "sample.kt")
    calls = _calls(r)
...
def test_swift_extension_across_files_merges_into_canonical_type():
    ...
    from purpory.extract import extract
    paths = sorted((FIXTURES / "swift_cross_file").glob("*.swift"))
    r = extract(paths, cache_root=Path("/tmp/purpory-test-no-cache"))
...
from purpory.extract import extract_elixir
...
# ── Objective-C ──────────────────────────────────────────────────────────────
from purpory.extract import extract_objc
...
def test_objc_header_dispatch_routes_objc_not_c(tmp_path):
    ...
    from purpory.extract import _get_extractor, extract_objc as _eo, extract_c as _ec
...
from purpory.extract import extract_markdown
