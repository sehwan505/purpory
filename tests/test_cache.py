"""Tests for purpory/cache.py."""
import pytest
from pathlib import Path
from purpory.cache import file_hash, cache_dir, load_cached, save_cached, cached_files, clear_cache, _body_content
...
# so a committed ``purpory-out/cache/`` is portable across machines and
...
def test_save_cached_relativizes_source_file(tmp_path):
    ...
    import json
    from purpory.cache import save_cached, file_hash, cache_dir
...
def test_load_cached_absolutizes_source_file(tmp_path):
    ...
    from purpory.cache import save_cached, load_cached
...
def test_load_cached_passes_through_legacy_absolute_source_file(tmp_path):
    """Cache entries written by an older purpory (with absolute source_file
    inside) must still load correctly: the absolutize step is a no-op for
    already-absolute values."""
    import json
    from purpory.cache import load_cached, file_hash, cache_dir
...
def test_cache_portable_across_roots(tmp_path):
    ...
    import json
    import shutil
    from purpory.cache import save_cached, load_cached, file_hash, cache_dir
...
# are only valid for the purpory version that wrote them. Keying purely on
...
def test_ast_cache_invalidated_on_version_bump(tmp_path, monkeypatch):
    ...
    import purpory.cache as cache_mod
    ...
    assert load_cached(f, root=tmp_path, kind="ast") is None, (
        "AST cache entry from a previous purpory version must not be served"
    )
...
def test_legacy_unversioned_ast_entries_not_served(tmp_path):
    """Entries written by pre-versioning purpory (flat cache/ or unversioned
    ...
    import json
    from purpory.cache import file_hash, _PURPORY_OUT
    ...
    unversioned = tmp_path / _PURPORY_OUT / "cache" / "ast"
...
def test_semantic_prune_removes_orphan_entries(tmp_path):
    ...
    current one."""
    from purpory.cache import prune_semantic_cache
...
def test_semantic_cache_deep_mode_roundtrip_under_deep_namespace(tmp_path):
    ...
    from purpory.cache import check_semantic_cache, save_semantic_cache
...
def test_semantic_cache_mode_none_layout_unchanged(tmp_path):
    ...
    h = file_hash(f, tmp_path)
    assert (tmp_path / "purpory-out" / "cache" / "semantic" / f"{h}.json").exists()
    assert not (tmp_path / "purpory-out" / "cache" / "semantic-deep").exists(), (
...
def test_clear_cache_removes_deep_namespace(tmp_path):
    ...
    save_semantic_cache([{"id": "d", "source_file": "doc.md"}], [],
                        root=tmp_path, mode="deep")
    base = tmp_path / "purpory-out" / "cache"
...
def test_semantic_prune_sweeps_both_namespaces_against_same_live_set(tmp_path):
    ...
    from purpory.cache import prune_semantic_cache, save_semantic_cache
...
def test_prompt_fingerprint_stable_and_prompt_sensitive(tmp_path):
    ...
    from purpory.cache import prompt_fingerprint
...
def test_semantic_cache_prompt_namespaced_layout(tmp_path):
    ...
    from purpory.cache import prompt_fingerprint, save_semantic_cache
    ...
    sem = tmp_path / "purpory-out" / "cache" / "semantic"
    h = file_hash(f, tmp_path)
...
def test_semantic_cache_prompt_and_mode_compose(tmp_path):
    ...
    deep = tmp_path / "purpory-out" / "cache" / "semantic-deep"
...
def test_semantic_cache_merge_existing_never_fuses_legacy_vintage(tmp_path):
    ...
    from purpory.cache import load_cached, save_semantic_cache
...
def test_semantic_prune_and_clear_reach_fingerprint_subdirs(tmp_path):
    ...
    save_semantic_cache([{"id": "n", "source_file": "doc.md"}], [],
                        root=tmp_path, prompt="PROMPT V1")
    clear_cache(tmp_path)
    assert not list((tmp_path / "purpory-out" / "cache" / "semantic").glob("**/*.json"))
