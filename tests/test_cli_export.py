"""Integration tests for purpory export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/purpory-out/."""
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "purpory-out" / "graph.html"
    assert html.exists()
...
def test_export_wiki_creates_articles(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    wiki = tmp_path / "purpory-out" / "wiki"
    assert wiki.exists()
...
# ── purpory export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_purpory_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["PURPORY_OUT"] = custom_out.name
...
def test_path_uses_purpory_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_purpory_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_extract_out_does_not_pollute_corpus(tmp_path):
    ...
    purpory-out/ (cache, stat-index) inside the scanned corpus."""
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .purpory_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".purpory_labels.json"
...
# ── communities-fallback when .purpory_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .purpory_analysis.json. The full `purpory extract`
...
# Without these tests, `purpory export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .purpory_analysis.json is absent, export html should reconstruct
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".purpory_analysis.json").read_text(encoding="utf-8"))
