...
import pytest
from purpory.hooks import install, uninstall, status, _hooks_dir, _HOOK_MARKER, _CHECKOUT_MARKER
...
def test_hook_skips_head_on_exe():
    ...
    from purpory.hooks import _PYTHON_DETECT
...
def test_install_embeds_pinned_interpreter(tmp_path):
    ...
    purpory launcher on PATH (uv tool / pipx isolation, #1127).

    When purpory is installed via `uv tool install purpory` or `pipx install
    purpory`, the interpreter lives in an isolated venv and the launcher is in
    ~/.local/bin.  GUI git clients and CI runners often run with a minimal PATH
    that omits that directory, so `command -v purpory` fails, the python3/python
    fallbacks cannot import purpory (wrong venv), and the hook silently exits 0.
...
from purpory.hooks import (  # noqa: E402
    _HOOK_SCRIPT,
    _CHECKOUT_SCRIPT,
...
def test_hooks_limit_windows_workers_by_default(name, script):
    ...
    assert 'export PURPORY_MAX_WORKERS="${PURPORY_MAX_WORKERS:-1}"' in script


def _launcher_payload(script: str) -> str:
    """Extract the `python -c "<payload>"` the hook hands to PURPORY_PYTHON.
...
def test_rebuild_bodies_read_purpory_root(name, body):
    """The rebuild must honour the persisted scan root rather than hardcoding the
    repo top (#1173). Both bodies read <output-dir>/.purpory_root and pass the
    recovered root to _rebuild_code instead of the bare Path('.')."""
    assert ".purpory_root" in body, f"{name} ignores .purpory_root (#1173)"
    # The output dir is resolved from PURPORY_OUT at hook-run time, not hardcoded
    # to purpory-out/, so a renamed output dir is still found (#1423).
    assert "PURPORY_OUT" in body, f"{name} ignores the PURPORY_OUT override (#1423)"
...
def test_rebuild_bodies_with_purpory_root_are_valid_python():
    """The .purpory_root snippet must parse so a quoting slip can't ship a hook
...
def test_detached_launch_targets_purpory_python():
    ...
    assert snippet.startswith('"$PURPORY_PYTHON" -c "')
...
def test_windows_hookspath_rejected_no_junk_dir_on_posix(tmp_path, monkeypatch, winpath):
    ...
    monkeypatch.setattr("purpory.hooks.os.name", "posix")
    repo = _make_git_repo(tmp_path)
...
def test_probes_use_find_spec_not_full_import():
    """`python -c "import purpory"` executes the FULL package import — 10s+ on a
    ...
    assert '-c "import purpory"' not in _PYTHON_DETECT, (
...
def test_shebang_read_is_null_byte_safe():
    """On Windows, `command -v purpory` can return the launcher path WITHOUT its
...
def test_probe_prefers_sibling_python_exe_on_windows_layouts():
    """pip on Windows puts Scripts/purpory(.exe) beside ..\\python.exe (or
...
def test_hooks_honor_skip_env(name, script):
    """PURPORY_SKIP_HOOK=1 must suppress BOTH hooks. post-checkout previously
    lacked the check, so the var stopped commit rebuilds but not branch-switch
    ones (#1809)."""
    assert '[ "${PURPORY_SKIP_HOOK:-0}" = "1" ] && exit 0' in script, (
        f"{name} does not honor PURPORY_SKIP_HOOK"
    )
...
def _worktree_guard_snippet() -> str:
    from purpory.hooks import _WORKTREE_GUARD
...
def test_install_registers_merge_driver(tmp_path):
    """install() must set merge.purpory.* via git config and add the
    .gitattributes line that README/CHANGELOG 0.7.0 document (#1902)."""
    repo = _make_git_repo(tmp_path)
    result = install(repo)
    res = subprocess.run(
        ["git", "-C", str(repo), "config", "--get", "merge.purpory.driver"],
        capture_output=True,
        text=True,
    )
    ...
    assert any("graph.json" in line and "merge=purpory" in line for line in attrs.splitlines())
...
def test_install_merge_driver_idempotent(tmp_path):
    ...
    matches = [l for l in lines if "merge=purpory" in l]
...
def test_install_preserves_existing_gitattributes(tmp_path):
    ...
    assert "merge=purpory" in content


def test_uninstall_removes_merge_driver_keeps_other_attrs(tmp_path):
    """uninstall() must unset merge.purpory.* and remove only the purpory
    ...
    content = (repo / ".gitattributes").read_text(encoding="utf-8")
    assert "*.png binary" in content
    assert "merge=purpory" not in content
