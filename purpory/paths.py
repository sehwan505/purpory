"""Single source of truth for the purpory output-directory name.

The output directory is ``purpory-out`` by default and overridable with the
``PURPORY_OUT`` env var (worktrees or shared-output setups, #686). It accepts a
relative name (``"purpory-out-feature"``) or an absolute path
(``"/shared/purpory-out"``).
...
``PURPORY_OUT`` before the process starts (the normal worktree/shared-output
...
PURPORY_OUT = os.environ.get("PURPORY_OUT", "purpory-out")
...
def _atomic_replace(path: "str | Path", write_fn) -> None:
    ...
    try:
        ...
        try:
            ...
            # back to copy-then-delete, matching purpory.cache's atomic writer.
            import shutil
...
# Bare directory name even when PURPORY_OUT is an absolute path. Used by the
...
PURPORY_OUT_NAME = os.path.basename(os.path.normpath(PURPORY_OUT))


def out_path(*parts: str) -> Path:
    """A path inside the configured output dir, e.g. ``out_path("cache")``.

    ``Path(PURPORY_OUT) / ...`` resolves correctly for both a relative name
    ("purpory-out") and an absolute override ("/shared/purpory-out").
    """
    return Path(PURPORY_OUT, *parts)
...
def default_graph_json() -> str:
    ...
    commands so a ``PURPORY_OUT`` override is honoured everywhere, not just where
