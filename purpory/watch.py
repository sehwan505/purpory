...
# Single source of truth in purpory.paths (#1423); re-exported as _PURPORY_OUT.
...
_BUILD_CONFIG_FILENAME = ".purpory_build.json"
...
def _apply_resource_limits() -> None:
    ...
    PURPORY_REBUILD_MEMORY_LIMIT_MB caps RSS-ish memory. Uses RLIMIT_DATA on
    ...
    try:
        ...
        pass
    mb = os.environ.get("PURPORY_REBUILD_MEMORY_LIMIT_MB", "").strip()
    if not mb:
        return
    try:
...
def _check_shrink(
    ...
    *,
    ...
    plain ``purpory update`` after deleting a function refresh the graph without
    ...
    if rebuilt_sources is not None:
        from purpory.build import _norm_source_file
...
def _stabilize_rebuild_cwd(watch_path: Path) -> bool:
    ...
    repo_root = os.environ.get("PURPORY_REPO_ROOT", "").strip()
