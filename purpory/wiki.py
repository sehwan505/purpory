...
def _community_article(
    ...
    lines += [
        "---",
        "",
        f"*Part of the purpory knowledge wiki. See {_md_link('index', resolver)} to navigate.*",
    ]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by purpory. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [purpory](https://github.com/safishamsi/purpory)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `purpory extract .` or `purpory cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `purpory extract .` to regenerate .purpory_analysis.json."
        )

    # Clear stale .md files from previous runs to prevent orphan accumulation.
    # Community labels are LLM-generated and non-deterministic
