...
from purpory.google_workspace import (
    GOOGLE_WORKSPACE_EXTENSIONS,
    convert_google_workspace_file,
    google_workspace_enabled,
)
from purpory.paths import PURPORY_OUT, PURPORY_OUT_NAME, out_path
...
# attacker-controllable (purpory runs on cloned/shared folders), and .docx/.xlsx
...
def classify_file(path: Path) -> FileType | None:
    ...
    from purpory.manifest_ingest import is_package_manifest_path
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the purpory extract pipeline.
...
_SKIP_DIRS = {
    ...
    "purpory-out", PURPORY_OUT_NAME,  # never treat own output as source input (#524); honour PURPORY_OUT (#1423)
    ...
    ".purpory",  # purpory's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .purporyignore file per gitignore spec.
...
def _git_info_exclude(vcs_root: Path) -> Path | None:
    ...
    a directory without any ``.gitignore`` entry. purpory only read
    ``.gitignore``/``.purporyignore``, so it walked into those worktree copies and
...
def _load_dir_own_ignore(d: Path) -> list[tuple[Path, str]]:
    """Read .gitignore/.purporyignore directly inside *d* (not its ancestors).

    Merges .gitignore and .purporyignore for this one directory (#1363):
    .gitignore is read first and .purporyignore last, so .purporyignore
    patterns (including `!` negations) win on conflict via last-match-wins;
    adding a .purporyignore can only ever exclude MORE, never re-include a
    .gitignore-excluded file (#945 kept: a dir with only a .gitignore still
    gets sensible defaults).

    Shared by `_load_purporyignore` (ancestor chain, loaded once before the
    ...
    for fname in (".gitignore", ".purporyignore"):
        ignore_file = d / fname
...
def _load_purporyignore(root: Path) -> list[tuple[Path, str]]:
    """Read .purporyignore files and return (anchor_dir, pattern) pairs.
    ...
    # per-directory .gitignore/.purporyignore — so load it first (lowest priority
...
def _is_ignored(
    ...
    *,
    ...
    """Return True if the path should be ignored per .purporyignore patterns.
...
def _load_purporyinclude(root: Path) -> list[tuple[Path, str]]:
    """Read .purporyinclude allowlist patterns from root and ancestors.
    ...
    Uses the same VCS-root ceiling logic as _load_purporyignore.
    """
    root = root.resolve()
    ceiling = _find_vcs_root(root) or root
...
def _is_included(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .purporyinclude allowlist pattern."""
    if not patterns:
        return False
...
def _could_contain_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if a directory may contain files matched by .purporyinclude."""
    if not patterns:
        return False
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None) -> dict:
    ...
    def _wc(path: Path) -> int:
        ...
        from purpory import cache as _cache
        return _cache.cached_word_count(path, root, count_words, cache_root=cache_root)

    skipped_sensitive: list[str] = []
    unclassified: list[str] = []
    # Files/dirs dropped by a .gitignore/.purporyignore rule. Recorded so an
    ...
    ignored: list[str] = []
    ignore_patterns = _load_purporyignore(root)
    ...
    # so they win over any .purporyignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
                ignore_patterns.append((root, line))
    include_patterns = _load_purporyinclude(root)

    # Always include purpory-out/memory/ - query results filed back into the graph
    memory_dir = root / PURPORY_OUT / "memory"
    scan_paths = [root]
    ...
    def _on_walk_error(err: OSError) -> None:
        ...
        print(
            f"[purpory] WARNING: could not scan {target} ({err}); "
    ...
    converted_dir = root / PURPORY_OUT / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set PURPORY_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install purpory[office]]")
                continue
    ...
    return {
        ...
        "purporyignore_patterns": len(ignore_patterns),
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`purpory.watch._relativize_source_files` so the
...
def load_manifest(
    ...
    *,
    ...
    manifests with absolute keys pass through unchanged, so a purpory-out/
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `purpory update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `purpory extract` after semantic extraction.
    ...
    .purporyignore/.gitignore/--exclude) are dropped instead of surviving
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `purpory extract` so that
        files touched by `purpory update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `purpory update`.
