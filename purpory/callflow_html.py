#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from purpory knowledge graph outputs.

Reads graph.json plus optional GRAPH_REPORT.md, .purpory_labels.json, and sections JSON,
...
Usage:
  python3 -m purpory export callflow-html
  python3 -m purpory export callflow-html /path/to/project/purpory-out/graph.json
  python3 -m purpory export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
from purpory.paths import PURPORY_OUT, PURPORY_OUT_NAME
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize a purpory node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize purpory edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current purpory graph.json via NetworkX's node-link parser."""
...
def load_labels(path: str | Path | None) -> dict:
    """Load community labels from .purpory_labels.json, tolerating wrapper keys."""
...
def infer_project_name(graph_path: str, meta: dict) -> str:
    ...
    if path.parent.name == PURPORY_OUT_NAME and len(path.parents) > 1:
        return path.parents[1].name
    return path.parent.name or "Project"


def resolve_purpory_paths(args) -> dict:
    """Resolve project root, purpory output dir, and optional files."""
    base = Path(args.project).expanduser() if args.project else Path.cwd()
    if args.purpory_out:
        purpory_out = Path(args.purpory_out).expanduser()
    elif args.graph:
        purpory_out = Path(args.graph).expanduser().parent
    elif (base / "graph.json").exists():
        purpory_out = base
    else:
        purpory_out = base / PURPORY_OUT

    project_root = purpory_out.parent if purpory_out.name == PURPORY_OUT_NAME else base
    graph = Path(args.graph).expanduser() if args.graph else purpory_out / "graph.json"
    report = Path(args.report).expanduser() if args.report else purpory_out / "GRAPH_REPORT.md"
    labels = Path(args.labels).expanduser() if args.labels else purpory_out / ".purpory_labels.json"
    sections = Path(args.sections).expanduser() if args.sections else None
    return {
        "base": project_root,
        "purpory_out": purpory_out,
        "graph": graph,
        "report": report,
        "labels": labels,
        "sections": sections,
...
def node_importance(node: dict) -> float:
    """Use purpory centrality fields when available."""
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 purpory 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from purpory knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 purpory 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from purpory community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
class CallflowOptions:
    ...
    def __init__(
        ...
        *,
        ...
        self.purpory_out = str(purpory_out) if purpory_out is not None else None
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from purpory output files."""
    ...
    if args.output:
        ...
        output_path = paths["purpory_out"] / f"{safe_filename(meta['project_name'])}-callflow.html"
...
""")
    ...
    overview_name = (
        sections[0].get("name", "Architecture Overview") if sections else "Architecture Overview"
    )
...
""")
    html.append(
        generate_overview_graph(
            sections, section_nodes_map, classified, labels, lang, args.diagram_scale
        )
    )
    html.append("""</div>
""")
    html.append(
        generate_overview_cards(meta, report_text, sections, section_nodes_map, classified, lang)
    )
...
""")
  ...
  <p>Generated: {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")} · purpory callflow-html</p>
</div>
""")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from purpory knowledge graph outputs"
    )
    parser.add_argument(
        "project", nargs="?", default=None, help="Project root or purpory output directory"
    )
    parser.add_argument("--purpory-out", default=None, help="Path to purpory output directory")
    ...
    parser.add_argument("--labels", default=None, help="Path to .purpory_labels.json")
    parser.add_argument(
