"""Deterministic "work memory" reflection over purpory-out/memory/.

`purpory reflect` reads the Q&A memory docs that `purpory save-result` files back
...
and a given ``now``. When a graph (`graph.json` + `.purpory_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``purpory-out/reflections/LESSONS.md`` rather than inside the wiki
because ``purpory export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from purpory.ingest import OUTCOMES
from purpory.paths import PURPORY_OUT_NAME
...
LEARNING_SIDECAR_NAME = ".purpory_learning.json"
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `purpory export wiki` reads graph.json + .purpory_analysis.json +
    .purpory_labels.json. Community membership in the analysis sidecar is keyed by
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `purpory reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in purpory-out/memory/. "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``purpory reflect --if-stale`` skip a redundant run — e.g. when the git
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_data is not None:
        ...
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".purpory_labels.json")
...
# graph.json as ``.purpory_learning.json``. It carries which nodes have proven
...
def _resolve_source_path(src: str, graph_path: Path) -> Path | None:
    ...
    live in ``<root>/purpory-out/`` (so its own dir is not the root) or directly
    ...
    Order: the committed ``.purpory_root`` marker (#686/#1423 — authoritative for
    an absolute/elsewhere ``PURPORY_OUT`` override); then the layout-appropriate
    root *first* — graph.json's parent's parent for the ``purpory-out`` layout,
    ...
    try:
        recorded = (out_dir / ".purpory_root").read_text(encoding="utf-8").strip()
        if recorded:
    ...
    if out_dir.name == PURPORY_OUT_NAME:
...
def write_learning_sidecar(agg: dict[str, Any], graph_path: Path,
    ...
    """Write ``.purpory_learning.json`` next to ``graph_path`` deterministically.
