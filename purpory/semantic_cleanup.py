...
#
# Called by external semantic merge workflows and the in-process
# `purpory merge-chunks` command — both ingest untrusted agent-written chunk
...
# external agent workflows require a Python enforcement boundary so a
# malicious or runaway agent response cannot exhaust memory or escape the
# purpory-out chunk directory via crafted node/edge IDs.
...
def validate_semantic_fragment(fragment: object) -> list[str]:
    ...
    Empty list means valid. Called by semantic merge code before
