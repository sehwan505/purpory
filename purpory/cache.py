...
# Output directory name — override with PURPORY_OUT env var for worktrees or
# shared-output setups. Accepts a relative name ("purpory-out-feature") or an
# absolute path ("/shared/purpory-out"). Single source of truth in purpory.paths
# (#1423); re-exported here as _PURPORY_OUT for the existing call sites.
from purpory.paths import PURPORY_OUT as _PURPORY_OUT

# AST cache entries are the output of purpory's own extractor code, so they
...
try:
    ...
    _EXTRACTOR_VERSION = _pkg_version("purpory")
except Exception:
...
def _cleanup_stale_ast_entries(ast_base: Path, current_dir: Path) -> None:
    """Remove AST cache entries left behind by other purpory versions.
...
def prompt_fingerprint(prompt: "str | Path") -> str:
    ...
    its system prompt, :func:`purpory.llm._extraction_system`) or a Path to a
    prompt file loaded by an external extraction workflow.
...
def _resolve_prompt_fp(
    ...
    because file-backed callers may substitute a path into a command — passing
    that path as ``prompt`` would hash the path
    ...
    fallback silently restores the very behavior this fixes.
    """
    memo_key = None
...
# `purpory extract --force` / `purpory update --force` (or PURPORY_FORCE=1)
...
def _ensure_stat_index(root: Path, cache_root: "Path | None" = None) -> None:
    ...
    # scanned corpus with a stray purpory-out/ (#1747).
...
def file_hash(path: Path, root: Path = Path("."), cache_root: "Path | None" = None) -> str:
    ...
    # purpory-out/cache/stat-index.json inside the analyzed source tree even when
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`purpory.watch._relativize_source_files` so cached
    ...
    :func:`purpory.detect._to_relative_for_storage`.
    """
    try:
...
def cache_dir(root: Path = Path("."), kind: str = "ast", prompt_fp: str | None = None) -> Path:
    ...
    AST entries live in purpory-out/cache/ast/v{version}/ — namespaced by
    purpory version because they depend on extractor code, not just file
    contents. Semantic entries are still NOT version-namespaced (re-extraction
    costs LLM calls, #1252): they live in purpory-out/cache/semantic/, with
    deep-mode entries beside them in purpory-out/cache/semantic-deep/.
...
def load_cached(
    ...
    Cache value: stored as purpory-out/cache/{kind}/{hash}.json (AST entries
    ...
    AST entries written by other purpory versions — including the legacy
...
def save_cached(
    ...
    Stores as purpory-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
    ...
    if isinstance(result, dict) and any(
        result.get(k) for k in ("nodes", "edges", "hyperedges", "raw_calls")
    ):
...
def cached_files(root: Path = Path(".")) -> set[str]:
    ...
    base = Path(root).resolve() / _PURPORY_OUT / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
...
def prune_semantic_cache(
    ...
    *,
    ...
    extraction-spec) can share one purpory-out/, and a wholesale sweep would
...
def check_semantic_cache(
    ...
    files — the prompt text (Python path) or a Path to an externally managed
    prompt file. Supplying it
    ...
    if legacy:
        warnings.warn(
            ...
            "vintages. Re-run with --force (or PURPORY_FORCE=1) to re-extract them "
