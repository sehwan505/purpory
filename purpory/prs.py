"""purpory prs — graph-aware PR dashboard.
...
Usage:
  purpory prs                   # dashboard of all open PRs
  purpory prs <number>          # deep dive on one PR
  purpory prs --triage          # rank the review queue from current PR state
  purpory prs --worktrees       # show worktree → branch → PR mapping
  purpory prs --conflicts       # PRs sharing graph communities (merge-order risk)
  purpory prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  purpory prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def render_triage(prs: list[PRInfo], base: str) -> None:
    ...
    if not candidates:
        ...
        return
    print()
    print(bold("  Triage") + dim(" (deterministic)"))
    print()
    order = {
        status: index
        for index, status in enumerate(
            ("CI-FAIL", "CHANGES-REQ", "READY", "PENDING", "APPROVED", "DRAFT")
        )
    }
    for priority, pr in enumerate(
        sorted(candidates, key=lambda item: (order.get(item.status, 99), -item.nodes_affected, item.number)),
        1,
    ):
        action = {
            "CI-FAIL": "fix failing CI",
            "CHANGES-REQ": "address requested changes",
            "DRAFT": "finish the draft",
            "PENDING": "wait for CI",
            "APPROVED": "merge when ready",
            "READY": "review now",
        }.get(pr.status, "inspect")
        impact = f" · {pr.blast_radius}" if pr.blast_radius else ""
        print(f"  {priority}. #{pr.number} — {action} [{pr.status}]{impact}")
