...
Usage:
  ...
  purpory prs --triage          # Opus ranks your review queue
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using PURPORY_TRIAGE_BACKEND or first available key."""
    from purpory.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("PURPORY_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("PURPORY_TRIAGE_MODEL")
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from purpory.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  purpory.llm not available - cannot run triage."), file=sys.stderr)
        sys.exit(1)
