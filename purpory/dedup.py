"""Entity deduplication pipeline for purpory knowledge graphs.
...
from purpory._minhash import MinHash, MinHashLSH
    ...
    for loser in losers:
        ...
        if lose_file == keep_file:
            if _norm(lose_label) != _norm(keep_label):
                print(
                    f"[purpory] note: node '{nid}' was extracted twice from "
            ...
            print(
                f"[purpory] WARNING: node '{nid}' is minted by two different files — "
...
def deduplicate_entities(
    ...
    *,
    ...
    total = len(remap)
    msg = f"[purpory] Deduplicated {total} node(s)"
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from purpory.llm import BACKENDS, _format_backend_env_keys, _get_backend_api_key
        if backend not in BACKENDS:
            print(f"[purpory] --dedup-llm: unknown backend {backend!r}, skipping LLM tiebreaker.", flush=True)
            return
        if not _get_backend_api_key(backend):
            env_keys = _format_backend_env_keys(backend)
            print(f"[purpory] --dedup-llm: {env_keys} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        ...
        # didn't exist in `purpory.llm` at all, so `--dedup-llm` was a no-op.
        # Surface the import failure so future regressions are visible.
        print(
            f"[purpory] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[purpory] --dedup-llm batch failed: {exc}", flush=True)
