...
from purpory.exporters.graphdb import push_to_falkordb, push_to_neo4j  # noqa: E402,F401
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .purpory_semantic_marker is present (graph cost real LLM tokens), or
    ...
    is_semantic = (out / ".purpory_semantic_marker").exists()
    is_curated = False
...
def _yaml_str(s: str) -> str:
    ...
    See `purpory.ingest._yaml_str` for the full rationale; duplicated here to
...
from purpory.exporters.base import COMMUNITY_COLORS  # noqa: E402,F401

from purpory.exporters.html import to_html  # noqa: E402,F401
        ...
        if not oversized:
            ...
            if not raw.strip():
                ...
                try:
                    ...
                    print(
                        f"[purpory] WARNING: existing {existing_path} could not be "
            ...
            if new_n < existing_n:
                ...
                print(
                    f"[purpory] WARNING: new graph has {new_n} nodes but existing "
                    ...
                    f"Run a full rebuild (/purpory .) to be safe, or pass force=True "
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /purpory", ""]
...
# Keep the backward-compatible alias for external callers.
...
def to_obsidian(
    ...
    # .obsidian/ config. Track the files purpory owns in a manifest; a pre-existing
    # file NOT in the manifest is the user's and is never overwritten.
    _manifest_path = out / ".purpory_obsidian_manifest.json"
    try:
        _owned: set[str] = set(
    ...
    def _owned_write(rel_name: str, content: str) -> bool:
        """Write a purpory-owned file, refusing to overwrite a pre-existing file
        purpory didn't create. Returns True if written."""
        target = out / rel_name
    ...
    # Map file_type → purpory tag
    _FTYPE_TAG = {
        "code": "purpory/code",
        "document": "purpory/document",
        "paper": "purpory/paper",
        "image": "purpory/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"purpory/{ftype}" if ftype else "purpory/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"purpory/{dom_conf}"
    ...
    # view — but never clobber an existing .obsidian/graph.json purpory doesn't own
    ...
    # manifest says purpory owns are candidates, and anything written or skipped
    ...
    if pruned:
        print(
            f"[purpory] pruned {pruned} note(s) for nodes no longer in the graph",
            file=sys.stderr,
        )

    # Persist the manifest of files purpory owns, so a re-run can safely update its
    ...
    if _skipped:
        ...
        print(
            f"[purpory] WARNING: skipped {len(_skipped)} pre-existing file(s) purpory "
            f"did not create, to avoid overwriting your notes: {shown}. "
            f"Export into an empty directory (or the default purpory-out/obsidian) "
