# git hook integration - install/uninstall purpory post-commit and post-checkout hooks
...
_HOOK_MARKER = "# purpory-hook-start"
_HOOK_MARKER_END = "# purpory-hook-end"
_CHECKOUT_MARKER = "# purpory-checkout-hook-start"
_CHECKOUT_MARKER_END = "# purpory-checkout-hook-end"

# __PINNED_PYTHON__ is replaced at install time with the absolute path of the
# Python interpreter that ran `purpory hook install`.  For uv-tool and pipx
...
# when the purpory launcher is not on PATH (common in GUI clients and CI).
#
# Probes check availability with importlib.util.find_spec instead of importing
# the package: a probe that imports purpory wholesale executes the full package
...
_GFY_PROBE="import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('purpory') else 1)"
PURPORY_PYTHON=""
...
if [ -z "$PURPORY_PYTHON" ]; then
    ...
    if [ -n "$PURPORY_BIN" ]; then
        # Windows pip layout: Scripts/purpory(.exe) sits beside ..\\python.exe
    ...
    if [ -z "$PURPORY_PYTHON" ] && [ -n "$PURPORY_BIN" ]; then
        ...
        case "$PURPORY_BIN" in
            *.exe) _SHEBANG="" ;;
            *)     _SHEBANG=$(head -c 256 "$PURPORY_BIN" 2>/dev/null | tr -d '\\000' | head -n 1 | sed 's/^#![[:space:]]*//') ;;
        esac
        case "$_SHEBANG" in
            */env\\ *) PURPORY_PYTHON="${_SHEBANG#*/env }" ;;
            *)         PURPORY_PYTHON="$_SHEBANG" ;;
        esac
        # Allowlist: only keep characters valid in a filesystem path to prevent
        # injection if the shebang contains shell metacharacters.
        case "$PURPORY_PYTHON" in
            *[!a-zA-Z0-9/_.@-]*) PURPORY_PYTHON="" ;;
        esac
        if [ -n "$PURPORY_PYTHON" ] && ! "$PURPORY_PYTHON" -c "$_GFY_PROBE" 2>/dev/null; then
            PURPORY_PYTHON=""
        fi
    fi
fi
...
if [ -z "$PURPORY_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "$_GFY_PROBE" 2>/dev/null; then
        PURPORY_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "$_GFY_PROBE" 2>/dev/null; then
        PURPORY_PYTHON="python"
    else
        echo "[purpory hook] could not locate a Python with purpory installed. Add the purpory bin dir to PATH or re-run 'purpory hook install' from the env where purpory lives." >&2
        exit 0
    fi
fi
"""
...
changed_raw = os.environ.get('PURPORY_CHANGED', '')
changed = [Path(f.strip()) for f in changed_raw.strip().splitlines() if f.strip()]

if not changed:
    sys.exit(0)

print(f'[purpory hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from purpory.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('PURPORY_REBUILD_TIMEOUT', '600'))
    ...
    print(f'[purpory hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    print(f'[purpory] Rebuild failed: {exc}')
    sys.exit(1)
"""

# Cross-platform detached-launch shim (#1161). The hooks used to background the
# rebuild with `nohup "$PURPORY_PYTHON" -c "..." &`, but Git for Windows' bundled
...
# still returned 0, so the graph just went stale with no signal. purpory already
...
__REBUILD_BODY__
'''
_log = os.environ.get('PURPORY_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'purpory-rebuild.log')
try:
...
def _detached_launch(rebuild_body: str) -> str:
    ...
    Python process via ``$PURPORY_PYTHON``.
    ...
    The launcher writes the child's output to ``$PURPORY_REBUILD_LOG`` and
...
# Installed by: purpory hook install
...
# churn run-to-run. Pinning it makes purpory-out reproducible.
...
# explicit PURPORY_MAX_WORKERS still wins for users who want parallelism.
if [ -n "${WINDIR:-}" ] || [ -n "${MSYSTEM:-}" ]; then
    export PURPORY_MAX_WORKERS="${PURPORY_MAX_WORKERS:-1}"
fi
...
# Skip when only purpory-out/ artifacts changed (avoids rebuild loop when graph outputs are tracked in git)
_NON_GRAPH=$(echo "$CHANGED" | grep -v '^purpory-out/' || true)
if [ -z "$_NON_GRAPH" ]; then
    exit 0
fi

""" + _PYTHON_DETECT + """
export PURPORY_CHANGED="$CHANGED"
...
echo "[purpory hook] launching background rebuild (log: $_PURPORY_LOG)"
""" + _detached_launch(_REBUILD_BODY_COMMIT) + """# purpory-hook-end
"""


_CHECKOUT_SCRIPT = """\
# purpory-checkout-hook-start
...
# Honor the same opt-out as post-commit: without this, PURPORY_SKIP_HOOK=1
...
mkdir -p "$(dirname "$_PURPORY_LOG")"
export PURPORY_REBUILD_LOG="$_PURPORY_LOG"
echo "[purpory] Branch switched - launching background rebuild (log: $_PURPORY_LOG)"
""" + _detached_launch(_REBUILD_BODY_CHECKOUT) + """# purpory-checkout-hook-end
"""
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            print(
                f"[purpory hooks] git could not resolve the hooks path for "
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove purpory section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"purpory hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"purpory removed from {name} at {hook_path} (other hook content preserved)"
...
def install(path: Path = Path(".")) -> str:
    """Install purpory post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
    ...
    # Pin the current interpreter so the hook works even when the purpory
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove purpory post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if purpory hooks are installed."""
    root = _git_root(path)
    if root is None:
    ...
    def _check(name: str, marker: str) -> str:
        ...
        return "installed" if marker in p.read_text(encoding="utf-8") else "not installed (hook exists but purpory not found)"
