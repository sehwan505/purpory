...
#    passed to build().
#
# 3. Semantic merge: before calling build(), the extraction workflow merges cached
...
# Hyperedge member lists are canonically keyed `nodes` (see purpory/llm.py
...
def _infer_merge_root(graph_path: Path) -> str | None:
    ...
    Prefers the committed ``purpory-out/.purpory_root`` marker — the authoritative
    scan root purpory records at build/watch time (#686/#1423) — then falls back to
    the directory that contains the output dir (``graph.json``'s grandparent, i.e.
    ``<root>/purpory-out/graph.json`` -> ``<root>``). Returns None if neither
    resolves, in which case normalization is a no-op (prior behavior).
    """
    try:
        marker = graph_path.parent / ".purpory_root"
        if marker.exists():
...
def _semantic_id_remap(nodes: list, root: str | None) -> dict:
    ...
    from purpory.extractors.base import _file_stem  # local: avoid import cost at module load
...
def graph_has_legacy_ids(nodes: list, root: str | Path | None = None, sample: int = 300) -> bool:
    ...
    from purpory.extractors.base import _file_stem
...
def _doc_twin_remap(nodes: list) -> dict[str, str]:
    ...
    the same document. A ``purpory update`` after a semantic build leaves both,
...
def build_from_json(
    ...
    for node in extraction.get("nodes", []):
        ...
        if "source" in node and "source_file" not in node:
            ...
            print(
                f"[purpory] WARNING: node '{node_id}' uses field 'source' instead of "
        ...
        # purpory versions that didn't always populate file_type) don't
    ...
    if real_errors:
        print(
            f"[purpory] Extraction warning ({len(real_errors)} issues): {real_errors[0]}",
            file=sys.stderr,
        )
    ...
    for node in extraction.get("nodes", []):
        ...
        if isinstance(node, dict):
            ...
            try:
                ...
                print(
                    f"[purpory] WARNING: skipping node with non-hashable id "
    ...
    from purpory.extractors.base import _file_stem as _fs
    ...
    for edge in sorted(
        ...
        try:
            ...
            print(
                f"[purpory] WARNING: skipping edge with non-hashable endpoint "
        ...
        for he in hyperedges:
            ...
            if isinstance(he, dict) and isinstance(he.get("nodes"), list):
                ...
                if not valid_members:
                    print(
                        f"[purpory] WARNING: dropping hyperedge "
...
def build(
    ...
    *,
    ...
    """
    from purpory.dedup import deduplicate_entities
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[purpory] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
    ...
    *,
    ...
    if existing_hyperedges:
        ...
        for he in existing_hyperedges:
            ...
            carried.append(he)
        if carried:
            from purpory.export import attach_hyperedges
    ...
    if prune_sources:
        ...
        if n_nodes:
            print(
                f"[purpory] Pruned {n_nodes} node(s) from {n_files} deleted source file(s).",
                file=sys.stderr,
            )
        ...
        if edges_to_remove:
            ...
            print(
                f"[purpory] Pruned {len(edges_to_remove)} edge(s) from deleted source file(s).",
                file=sys.stderr,
            )

        if not n_nodes and not edges_to_remove:
            print(
                f"[purpory] {n_files} source file(s) deleted since last run — "
    ...
    if had_graph and not dedup and not prune_sources:
        ...
        if new_n < existing_n:
            raise ValueError(
                f"purpory: build_merge would shrink graph from {existing_n} → {new_n} nodes. "
...
def distinct_repo_tags(graph_paths: "list[Path]") -> "list[str]":
    ...
    """
    repo_dirs = [p.parent.parent for p in graph_paths]  # purpory-out/.. → repo dir
