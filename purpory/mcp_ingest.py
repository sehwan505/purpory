...
and turns the `mcpServers` map into Purpory nodes and edges.

Symmetry with `serve.py`: Purpory exposes itself AS an MCP server. This module
indexes MCP servers AS a corpus type, completing the loop — an agent that runs
purpory with `--mcp` can now query its own configured MCP layer.

Entry point:
  extract_mcp_config(path: Path) -> dict[str, list[dict]]

  Returns `{"nodes": [...], "edges": [...]}` compatible with Purpory's
...
def extract_mcp_config(path: Path) -> dict[str, Any]:
    """Parse an MCP config file into Purpory nodes and edges.
...
# ── Node / edge construction (Purpory schema) ────────────────────────────────
...
# only purpory.ids), so this can no longer drift from extract._file_stem.
from purpory.extractors.base import _file_stem  # noqa: E402
